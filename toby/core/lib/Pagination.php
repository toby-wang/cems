...
class Pagination
{
     ...
     * @param type $base_uri    url附加参数
     * @param type $pageParam  分页变量
     */
    public function init($total, $pageSize, $base_uri = '', $pageParam = 'page') {
        $this->total = intval($total);
        $this->pageSize = intval($pageSize);
        $this->pageParam = $pageParam;
        $this->uri = $this->geturi($base_uri);
     ...
     * @return string
     */
    private function geturi($base_uri) {
        /*
    ...
    private function frist() {
        $html = '';
        if ($this->page == 1) {
            $html .= $this->replace("{$this->uri}/1", $this->config['frist'], true);
        } else {
            $html .= $this->replace("{$this->uri}/1", $this->config['frist'], false);
        }
        return $html;
